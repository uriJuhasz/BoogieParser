...
namespace BoogieAST{
	...
	protected:
		BoogieNamedEntity(const std::string& _name) : name(_name){}
	};
	...
	public:
		BoogieIdentifier(const std::string& _id) : id(_id){}
		const std::string id;
	};
	class BoogieProgram : public BoogieScope{
	public:
		void addTypeDeclaration(std::unique_ptr<BoogieTypeDeclaration>& d){typeDeclarations.push_back(move(d));}
	private:
		std::map<std::string,BoogieTypeDeclaration> types;
		std::list<std::unique_ptr<BoogieTypeDeclaration>> typeDeclarations;
	};
